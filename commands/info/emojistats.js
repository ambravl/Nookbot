module.exports.run = (client, message, args) => {
  ...
  let msg = '';
  const emojiSize = client.emojiDB.count();
  ...
  switch (args[0]) {
    ...
    case 'bottom':
      ...
      client.emojiDB.map((v, k) => ({ id: k, uses: v }))
        ...
        .slice(-num1)
        .forEach((e, i) => {
          msg += `\n#${emojiSize - num1 + i + 1} - ${message.guild.emojis.cache.get(e.id).name} - ${e.uses}`;
        });
    ...
    case 'less':
      ...
      client.emojiDB.map((v, k) => ({ id: k, uses: v }))
        ...
        .filter((e) => e.uses <= num1)
        .forEach((e, i, a) => {
          msg += `\n#${emojiSize - a.length + i + 1} - ${message.guild.emojis.cache.get(e.id).name} - ${e.uses}`;
        });
