  ...
  switch (args[0]) {
    ...
    default: {
      if (args.length === 0) {
        ...
        const fc = client.userDB.ensure(message.author.id, client.config.usersDefaults).friendCode;
        if (!fc) {
      ...
      if (member) {
        const fc = client.userDB.ensure(member.user.id, client.config.usersDefaults).friendCode;
        if (!fc) {
