...
module.exports.run = (client, message, args, level) => {
  ...
  const { posRepList, negRepList } = client.db.users.ensure(member.id, client.config.usersDefaults);
  ...
  if (negRepList.includes(message.author.id)) {
    client.db.users.math(member.id, '-', 1, 'negativeRep');
    client.db.users.remove(member.id, message.author.id, 'negRepList');
    client.db.users.math(member.id, '+', 1, 'positiveRep');
    client.db.users.push(member.id, message.author.id, 'posRepList');
